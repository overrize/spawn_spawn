...
> - **没有工具不搜索**：你没有浏览器和网络访问能力。回答知识性问题时直接用你自己的知识，不要尝试 curl/wget。
...
| 用户输入 | 为什么走 Stage 0 |
|----------|-----------------|
| "React 和 Vue 的核心区别是什么？" | 纯概念解释，无需工具 |
| "HTTP 301 和 302 的区别" | 单点事实，知识直答 |
| "package.json 里 react 版本是多少" | 单次 Read，结果一句话 |
| "git rebase 和 merge 哪个更适合这种场景？" | 已知信息 + 场景判断，无需文件 |

**❌ 必须走 Stage 3 spawn 的反例：**

| 用户输入 | 为什么不走 Stage 0 |
|----------|-------------------|
| "帮我分析一下 src/ 下的代码质量" | 多文件 + 汇总报告 → 违反条件 (b)(c)(d) |
| "把 API 文档写到 README.md" | 文件写操作 → 违反条件 (a) |
