...
| M3-7b-wire | **TUI 接线**：新视图层成为 App() 唯一渲染+输入，旧三栏 JSX/useInput 已删除；`InkView` 接 `dispatchUser`（斜杠命令+用户消息复用）、审批 onApprove/onReject（复用工具执行）、退出 | AgentRuntime.tsx App() 改造 + `src/tui/InkView.tsx` | typecheck 干净 + 全量 527/520/0 fail + test:full exit 0；**交互冒烟归用户**（`npm run dev:watch` 键位/审批/滚动） | M3-7b-view | 待用户交互验收 |
