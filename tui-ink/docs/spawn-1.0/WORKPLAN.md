...
| M3-2 | 评测集 ≥30 条端到端用例（任务拆分/工具准确率/协议遵守/收敛/记忆质量），QA-TL 执行器，demo/live 双模式 | eval 用例 + 执行器 | 评测集在 demo 模式全量可跑，live 抽样可跑 | M1 起持续积累 | 进行中（**案例 30/30 达标**：EV-001..030，27 具体 demo 全绿 + 3 递延 M1-6e；剩「live QA-TL 执行器」半） |
