...
- [x] **6a Observability** — Step-0 RuntimeContext + `ObservabilityRuntime` 门面（`logDiag`/可注入 sink/`installDiagnosticSink` + 指标 re-export）；55 处裸 stderr.write→`logDiag`；规格 `M1-6a-SPEC.md`；测试 `observability-runtime-6a.test.ts`（5 组绿）；golden 25 不变、549/0 fail、test:full exit 0。**待用户冒烟 S1–S6**
- [x] **6b Tool** — `ToolRuntime` 门面新增 `runToolBatch`/`formatToolResults`/`findApproverLeader`（纯函数三件套）；leader+worker 两处批执行与 combined 构造改走门面（worker 熔断/收束后缀原样保留在调用点）；`findApproverLeader` 移入并注入 `{agents,getState}`；规格 `M1-6b-SPEC.md`；测试 `tool-runtime-6b.test.ts`（4 组绿）；golden 25 不变、553/0 fail、test:full exit 0；I6 `executeTool(`=2（均审批后单发）。**待用户冒烟 S1–S6**。6e 债务：批执行 reinject 编排 + 审批分发 + toolQueue 生命周期
- [x] **6c Memory** — `MemoryRuntime` 门面 `initSecretary`/`destroySecretary`/`buildResumeContext`；secretary 创建/销毁去重 + resume 注入段抽出；规格 `M1-6c-SPEC.md`；测试 `memory-runtime-6c.test.ts`（6 组绿）；golden 25 不变、559/0 fail、test:full exit 0；I6 `new SecretaryProxy(`=0
- [x] **6d Orchestrator** — `OrchestratorRuntime` 门面 `checkMessageLegal`（通信矩阵）/`replaceAgentForResume`（resume 换手）依赖注入抽出；规格 `M1-6d-SPEC.md`；测试 `orchestrator-runtime-6d.test.ts`（3 组绿）；golden 25+pm-hierarchy 全绿、562/0 fail、test:full exit 0；I6=0。（`killAgent`+spawn-dispatch+parent-child 收敛留 6e）
- [x] **6e-3 Agent 处理器搬移** — DevHarness/FeishuBridge/commands + LeaderHandler(738)/WorkerHandler(806)/HandlerContext；**god-file 3965→1279，所有编排文件 <1500 达标**；golden 25+pm-hierarchy 60/0 fail、562/0 fail、test:full exit 0、零回归；F/C 段冒烟过
- [~] 6e-4 补 M0 递延断言（进行中）：**已完成** `test_failed_but_claimed_done` + `agent_done_blocked_by_guard`——抽 `decideDoneGuard` 接进 WorkerHandler 两个 agent.done 拦截（golden 25 确认等价，非并行死代码）+ `done-guard-6e4.test.ts` 5 组绿。**已转正** EV-011/012/013（测 AgentIdlePolicy）。**剩**：`hard_circuit_fired` + 把 `decideAgentIdleAfterNoTools` 接进 idle 逻辑（inline 与 policy 有分歧，需调和重构，golden 风险）+ H 段处理器冒烟
...
| M1-6a | 拆出 `ObservabilityRuntime`（TraceEvent/HealthMetric/坏例捕获）**含第0步 RuntimeContext 注入**（见 [M1-6-SPLIT-SPEC.md](M1-6-SPLIT-SPEC.md)、[M1-6a-SPEC.md](M1-6a-SPEC.md)） | RuntimeContext + 独立模块 | M1-5 快照不变 | M1-5, M0 | 完成（`logDiag`/可注入 sink/`installDiagnosticSink` + 指标门面；golden 25 不变、549/0 fail、test:full exit 0；待用户冒烟） |
| M1-6b | 拆出 `ToolRuntime`（tool.call→执行→回注、审批、schema 校验）（见 [M1-6b-SPEC.md](M1-6b-SPEC.md)） | 独立模块 | M1-5 快照不变；registry 测试全绿 | M1-5, M1-1 | 完成（纯函数三件套 `runToolBatch`/`formatToolResults`/`findApproverLeader`；golden 25 不变、553/0 fail、test:full exit 0；待用户冒烟。**6e 债务**：批 reinject 编排 + 审批分发 + toolQueue 生命周期 + M1-1 自纠计数） |
| M1-6c | 拆出 `MemoryRuntime`（SecretaryProxy、session、resume context、提取）（见 [M1-6c-SPEC.md](M1-6c-SPEC.md)） | 独立模块 | M1-5 快照不变；memory 测试全绿 | M1-5 | 完成（`initSecretary`/`destroySecretary`/`buildResumeContext`；golden 25 不变、559/0 fail、test:full exit 0；I6 `new SecretaryProxy(`=0；待用户冒烟。**6e 债务**：`secretary.observe/on/observeMessage/ingestChildMemory` 转发 glue） |
| M1-6d | 拆出 `OrchestratorRuntime`（spawn、parent-child、kill、resume 编排）（见 [M1-6d-SPEC.md](M1-6d-SPEC.md)） | 独立模块 | M1-5 快照不变；pm-hierarchy 全绿 | M1-5 | 完成（`checkMessageLegal`/`replaceAgentForResume` 依赖注入抽出；golden 25+pm-hierarchy 60/0 fail、562/0 fail、test:full exit 0；I6=0；待用户冒烟。**6e 债务**：`killAgent`（feishuForks/abortAgent 横切）+ spawn-dispatch 守卫 + parent-child 收敛） |
| M1-6e | 拆出 `AgentRuntime`（agent send、事件处理、terminal guard）；TUI 只剩渲染/输入/slash glue（**设计见 [M1-6e-SPEC.md](M1-6e-SPEC.md)，分 6e-1..4 四阶段**） | **验收改（原 index.tsx<1500 被 Goodhart）**：无单一编排文件 >1500 行 + 提取的策略必须接进活代码 | M1-5 快照不变；全量测试绿；**补 M0 递延断言**：`hard_circuit_fired` / `agent_done_blocked_by_guard` / `test_failed_but_claimed_done`（`resume_context_missing` 已随 6c 转正）；App 层需人工冒烟矩阵 | M1-6a..d | 进行中（**设计已就绪**：见 [M1-6e-SPEC.md](M1-6e-SPEC.md)。用户选「先搬模块级大块」。**已完成**：① `DevHarness.ts`（runDemo+runTestSuite，3965→3729）② `FeishuBridge.ts`（connectFeishu + 14 个 feishu* 状态 verbatim 搬出，注入 6 依赖 via `configureFeishuBridge`，8 处外部引用改访问器，3729→**3118**）；两步 golden 25+全量 562/0 fail+test:full exit 0 不回归。③ `tui/commands.ts`（`buildCommands(ctx)` 抽出 COMMANDS 命令表 + parseScheduleDelay/fmtKIndex，TDZ 靠 confirmEffort/requestExit 箭头包裹 + pmStarted 上移，3118→**2747**）；三步 golden 25+全量 562/0 fail+test:full exit 0 不回归。④ **处理器搬移+拆分（6e-3）**：startLeaderAgent→`LeaderHandler.ts`(738)、startWorker→`WorkerHandler.ts`(806)、共享注入 `HandlerContext.ts`、`AgentHandlers.ts` 变 barrel；buildSystemPrompt/killAgent 留 AgentRuntime 注入；6e-2 TurnState **跳过**（per-turn `let` 是函数局部，随函数搬走，无需先抽对象）。**god-file 2747→1279**。**验收达标**：所有编排文件 <1500（AgentRuntime 1279 / Worker 806 / Leader 738 / Feishu 673 / commands 453 / DevHarness 254）；golden 25+pm-hierarchy 60/0 fail、全量 562/0 fail、test:full exit 0、typecheck 干净、**零回归**。F/C 段已冒烟验过。**剩 6e-4**：补 M0 递延断言（`hard_circuit_fired`/`agent_done_blocked_by_guard`/`test_failed_but_claimed_done`，处理器已可 import 测）+ EV-011/012/013 转正 + H 段处理器人工冒烟 | 
...
| M2-10 | **worker 报 done 后不干净终止**（冒烟发现 2026-07-24）：worker 完成并 `message.to=leader` 汇报后仍被喂输入，连吐 4 次非 JSON 散文 `（agent 已完成任务）`→parse 失败→靠"plain-text agent.done 检测"合成才收敛（tui.log 133910-134513）。与"只读死循环"同族：worker 缺"已汇报即应 agent.done"的干净收敛。fix：worker 汇报父后若无 todo 应主动合成 agent.done，而非等再喂输入吐散文；PROMPT_HEALTH 连续 N 次"已完成"类散文即视为 done | worker done-after-report 收敛 | 复现用例：worker 汇报后一轮内 agent.done、无 parse-fail 洪水；golden 25 不变 | M1-6d | 未开始 |
...
| M3-4 | 坏例归档与回流：评测失败/线上告警 → 带上下文 bad case（事件切片 + session 片段）→ 标注回流评测集 | 归档管线 | 一条真实坏例走完全流程 | M0, M3-2 | 未开始（**已有真实坏例待归档**：2026-07-23 [openclaw-distillation.md](../design/openclaw-distillation.md) 幻觉调研——agent 把游戏引擎 OpenClaw 编成 AI 助手，可作 M3-4 首条端到端样例：调研类任务缺"来源可验证"约束 → 归档 → 回流评测集加"未找到项目须显式声明而非编造"用例） |
| M3-5 | 进化触发器（建议模式）：指标超阈值 → bad-case report → 可 dispatch QA-TL 分析；patch 仍需评测 + 审批。**学术对接：Reflexion（arXiv 2303.11366）＝本触发器的学术版**——「执行→评估→反思→写 episodic memory→修正」循环即"感知→评估→修复"闭环；[reproducibility-review.md](../papers/reproducibility-review.md) 判其为 Top-1 复制目标（A 档，1-2 天，只需 3-5 prompt 模板 + episodic buffer + 循环调度器，不改底层）。落地时把「反思写入 episodic memory」接到现有 MemoryStore/HealthMetric，作为 M3-5 触发后的分析动作 | 触发器 + Reflexion loop wrapper | 阈值触发测试；不自动合入的守卫测试；Reflexion 循环单测（执行→反思→修正记录可读回） | M3-4 | 未开始（已挂 Reflexion 学术依据） |
...
| M3-7c | **TUI 显示模型改造（用户定调）**：以**旧富 UI 视觉风格为基础**，把显示模型对齐 **Claude Code 滚动式**——用 Ink `<Static>` 把消息/事件一次性打进终端滚动历史（可回翻、不整屏刷新），底部只留紧凑活区（输入+状态+当前 agent 活动）；三视图操作模型沿用之前定的 | 改造 ui.tsx 对话区为 Static transcript + 紧凑活区 | `npm run dev:watch` 满屏 + 滚动流畅 + 富样式保留（用户交互验收）；width 基元复用 | M3-7a | 待用户交互验收（dev:watch） |
