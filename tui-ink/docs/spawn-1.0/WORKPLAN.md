...
| M3-7c | **TUI 显示模型改造（用户定调）**：以**旧富 UI 视觉风格为基础**，把显示模型对齐 **Claude Code 滚动式**——用 Ink `<Static>` 把消息/事件一次性打进终端滚动历史（可回翻、不整屏刷新），底部只留紧凑活区（输入+状态+当前 agent 活动）；三视图操作模型沿用之前定的 | 改造 ui.tsx 对话区为 Static transcript + 紧凑活区 | `npm run dev:watch` 满屏 + 滚动流畅 + 富样式保留（用户交互验收）；width 基元复用 | M3-7a | 未开始（**下一步**） |
